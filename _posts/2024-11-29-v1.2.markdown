...
<hr />
#### Featured Changes
* Fixed sniper sway so it behaves identically at any FPS between 30 and 240 ([view](https://youtu.be/zyoKX45bKkE){:target="_blank"})
* Fixed rendering of 'god rays' on Windows (e.g., from the mercenary flashlight). Previously, this required 3rd party wrappers to resolve
...
* Coop moves were sometimes still failing at 90 FPS, so they will now run at 60 FPS
* Deprecated `fps_host` and `fps_client`. Replaced with `fps_spy` and `fps_merc` as Mercenaries can run at 240 FPS, but Spies become more prone movement glitches the higher you go above 120 FPS.
...
#### Fixed 'god rays' on Windows
{% include carousel.html height="56" unit="%" duration="120" number="6" %}
