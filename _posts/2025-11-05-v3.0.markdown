...
*   Holding use whilst in slow mode (standing/crouched) will try to grab ledges when you fall off. This feature is important for ledge hanging on controller, as the spy only grabs a ledge when a single directional input is registered, which can be difficult to do with a thumbstick. On keyboard, it’s only needed when strafing off a ledge while holding two movement keys.
...
##### Input
*   Added an option to enable native controller support through the Reloaded settings.
...
*   Added the option to disable the Spy's lock-on snap when drawing the Shocker Gun.
